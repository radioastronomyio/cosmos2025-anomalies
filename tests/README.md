...
- exact eleven-table set, configured full paths/basenames, independently
...
- one database `transaction_timestamp()` shared by all eleven rows and the
...
- precommit rollback, exact zero/eleven ambiguous-commit classification,
...
schema, aligned rows in all eleven mirrors, a simulated successful commit with
